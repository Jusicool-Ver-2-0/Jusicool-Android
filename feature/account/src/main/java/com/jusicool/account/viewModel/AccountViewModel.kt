...
import com.jusicool.usecase.holding.ObserveHoldingListUseCase
import com.jusicool.usecase.holding.RefreshHoldingListUseCase
...
internal class AccountViewModel @Inject constructor(
    ...
    private val observeHoldingListUseCase: ObserveHoldingListUseCase,
    ...
    private val refreshHoldingListUseCase: RefreshHoldingListUseCase
) : ViewModel() {

    private val _accountUiState = MutableStateFlow<GetAccountUiState>(GetAccountUiState.Loading)
    val accountUiState = _accountUiState.asStateFlow()

    val holdingUiState: StateFlow<GetHoldingUiState> =
        observeHoldingListUseCase()
            .map { holdingType ->
                _cryptoHoldings.value = holdingType.cryptoHoldings
                _markets.value = extractMarketValueFromHolding(holdingType.cryptoHoldings)
                GetHoldingUiState.Success(holdingType.cryptoHoldings)
            }.onStart {
                GetHoldingUiState.Loading
            }.catch { e ->
                GetHoldingUiState.Error(e.message ?: "Unknown error")
            }.stateIn(
                scope = viewModelScope,
                started = SharingStarted.WhileSubscribed(5000),
                initialValue = GetHoldingUiState.Loading,
            )
    ...
    fun refreshHoldingList() = viewModelScope.launch {
        refreshHoldingListUseCase()
    }
