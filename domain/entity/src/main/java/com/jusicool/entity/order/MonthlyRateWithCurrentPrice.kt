...
data class MonthlyRateWithCurrentPrice(
    ...
    /** 전 종목 중 상승 종목의 비율 (0.0 ~ 100.0) */
    val positiveRatio: Double
        get() = markets.count { it.isPositive }.toDouble() / markets.size * 100

    /** 전 종목 중 하락 종목의 비율 (0.0 ~ 100.0) */
    val negativeRatio: Double
        get() = markets.count { it.isNegative }.toDouble() / markets.size * 100
    ...
    val totalProfitAmount: Double
        get() = _totalCurrentAmount - _totalPurchaseAmount
}
...
data class MarketWithCurrentPrice(
    ...
    val previousPrice: Double
        get() = if (rate != -1.0) currentPrice / (1 + rate) else 0.0
